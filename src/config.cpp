...
HostConfig LoadConfig()
{
    ...
    if (GetFileAttributesW(path.c_str()) == INVALID_FILE_ATTRIBUTES)
    {
        ...
        winrt::check_bool(WritePrivateProfileStringW(L"Settings", L"TargetExecutable", L"RobloxPlayerBeta.exe", path.c_str()));
        winrt::check_bool(WritePrivateProfileStringW(L"Settings", L"EnableDepth", L"1", path.c_str()));
    }
    ...
    wchar_t targetExe[256]{};
    GetPrivateProfileStringW(L"Settings", L"TargetExecutable", L"RobloxPlayerBeta.exe", targetExe, static_cast<DWORD>(std::size(targetExe)), path.c_str());
    
    int enableDepth = GetPrivateProfileIntW(L"Settings", L"EnableDepth", 1, path.c_str());
